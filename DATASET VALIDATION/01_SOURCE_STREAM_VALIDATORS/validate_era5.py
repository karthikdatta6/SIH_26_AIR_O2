...
def process_all_era5():
    ...
    # If all processed station parquets already exist, audit them directly
    existing_parquets = glob.glob(os.path.join(PROC_ERA5_DIR, "*_era5_hourly.parquet"))
    if len(existing_parquets) >= 10:
        print(f"  -> Found {len(existing_parquets)} existing processed ERA5 station parquets. Auditing...")
        era5_qc_records = []
        for pq in existing_parquets:
            stn_name = os.path.basename(pq).replace("_era5_hourly.parquet", "")
            df_stn = pd.read_parquet(pq)
            assert len(df_stn) >= 26000, f"Station {stn_name} has insufficient records: {len(df_stn)}"
            era5_qc_records.append({
                "station_id": stn_name,
                "total_hourly_records": len(df_stn),
                "start_timestamp": str(df_stn["timestamp_utc"].min()),
                "end_timestamp": str(df_stn["timestamp_utc"].max()),
                "mean_temperature_c": round(float(df_stn["era5_temperature_c"].mean()), 2),
                "mean_relative_humidity": round(float(df_stn["era5_relative_humidity"].mean()), 2),
                "mean_wind_speed": round(float(df_stn["era5_wind_speed"].mean()), 2),
                "mean_surface_pressure_hpa": round(float(df_stn["era5_surface_pressure_hpa"].mean()), 2)
            })
            print(f"     [AUDITED] {stn_name}: {len(df_stn):,} hourly rows, Mean Temp: {df_stn['era5_temperature_c'].mean():.1f}°C")
        ...
        print(f"[DONE] Verified all 10 ERA5 station parquets: {era5_qc_out}")
        return
