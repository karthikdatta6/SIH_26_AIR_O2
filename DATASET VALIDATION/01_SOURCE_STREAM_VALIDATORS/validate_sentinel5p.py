...
def process_all_sentinel5p():
    ...
    existing_parquets = glob.glob(os.path.join(PROC_S5P_DIR, "*_s5p_daily.parquet"))
    if len(existing_parquets) >= 10:
        print(f"  -> Found {len(existing_parquets)} existing processed Sentinel-5P daily parquets. Auditing...")
        all_qc = []
        for pq in existing_parquets:
            stn_name = os.path.basename(pq).replace("_s5p_daily.parquet", "")
            df_daily = pd.read_parquet(pq)
            assert len(df_daily) >= 500, f"Station {stn_name} has insufficient records: {len(df_daily)}"
            for prod in ["sat_NO2", "sat_CO", "sat_HCHO"]:
                if prod in df_daily.columns:
                    valid_cnt = df_daily[prod].notna().sum()
                    all_qc.append({
                        "station_id": stn_name,
                        "product": prod.replace("sat_", ""),
                        "expected_observations": len(df_daily),
                        "valid_observations": valid_cnt,
                        "missing_or_cloud_filtered": len(df_daily) - valid_cnt,
                        "success_rate_pct": round(valid_cnt / len(df_daily) * 100, 2)
                    })
            print(f"     [AUDITED] {stn_name}: {len(df_daily):,} daily rows (NO2 Valid: {df_daily['sat_NO2'].notna().sum():,})")
        ...
        print(f"[DONE] Verified all 10 Sentinel-5P daily parquets: {qc_report_path}")
        return
